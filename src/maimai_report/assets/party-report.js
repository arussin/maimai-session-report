...
function open(url){
  ...
  if(!child){status.textContent='The new tab was blocked. Download your player file, open maimai.party, and import it from Settings.';download.hidden=false;return;}
  status.textContent='Opening maimai.party…';let connected=false,port=null;const preparation=latest();
  const timeout=setTimeout(()=>{if(!connected){window.removeEventListener('message',receive);status.textContent='The transfer could not connect. Download the player file and import it from Settings on maimai.party.';download.hidden=false;}},25000);
...
function mount(){if(document.getElementById('open-maimai-party'))return;const root=make('section',undefined,'party-report-actions');root.setAttribute('aria-label','maimai.party');status=make('p','','party-transfer-status');status.setAttribute('role','status');download=make('button','Download player file');download.type='button';download.hidden=!config.enabled||!config.payload;download.onclick=save;
  const holder=make('div');holder.innerHTML=wordmark;const brand=holder.firstElementChild;brand.id='open-maimai-party';brand.href=origin+'/';brand.target='_blank';brand.rel='noopener';brand.setAttribute('aria-label','Open in maimai.party');brand.prepend(make('span','Open in','party-open-label'));brand.onclick=e=>{e.preventDefault();open(new URL(origin+'/'));};root.append(brand,download,status);document.getElementById('app').prepend(root);
}
