...
function mount(){if(document.getElementById('open-maimai-party'))return;const toolbar=document.querySelector('.toolbar');if(!toolbar)return;status=make('p','','party-transfer-status');status.setAttribute('role','status');download=make('button','Download player file','action-button');download.id='download-player-data';download.type='button';download.hidden=!config.enabled||!config.payload;download.onclick=save;
  const holder=make('div');holder.innerHTML=wordmark;const brand=holder.firstElementChild;brand.id='open-maimai-party';brand.href=origin+'/';brand.target='_blank';brand.rel='noopener';brand.setAttribute('aria-label','Open in maimai.party');const suffix=brand.querySelector('.party-suffix');suffix.querySelector('.party-dot').remove();brand.replaceChildren(make('span','Open in','party-open-label'),suffix);suffix.setAttribute('aria-hidden','true');brand.onclick=e=>{e.preventDefault();status.classList.remove('party-transfer-error');open(new URL(origin+'/'));};toolbar.querySelector('.report-brand-block').append(brand);const group=make('div',undefined,'party-download-group'),help=make('a','?','party-player-help');help.href='https://github.com/arussin/maimai-session-report/blob/main/docs/PLAYER_FILE.md';help.target='_blank';help.rel='noopener noreferrer';help.referrerPolicy='no-referrer';help.setAttribute('aria-label','About player files — Session Report guide');help.title='Save your scores and retained history for import in maimai.party. Read the player-file guide.';group.append(download,help);toolbar.querySelector('.report-downloads').append(group);toolbar.after(status);
}
