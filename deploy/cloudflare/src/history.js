...
export function historyPage({rows, total, cursor = '', hasMore = false, prefix = '/maimai/', support = {}}) {
  const body = rows.length ? `<section class="history-summary" aria-label="History summary"><div><h2>Rating over time</h2><p class="trend-caption">After each shown capture · Reconstructed Old 35 + New 15</p>${trend(rows)}</div><div class="coverage"><div><span class="stat-label">Retained sessions</span><p class="history-stat">${number(total)}</p></div><p>Open a date for its full scorecard, plays, rating pools and targets.</p></div></section><div class="list-heading"><h2>Session archive</h2><p>Newest first · ${rows.length} shown</p></div><ol class="capture-list">${rows.map((r,i) => {
    ...
    return `<li><a class="capture" href="${esc(prefix)}history/c/${r.id}/"><div class="identity"><h3>${esc(date(r.sort_ms,r.timezone))}</h3><small>${number(r.score_count)} retained plays · ${Number.isSafeInteger(r.before_rating) ? `${number(r.pb_count)} PB changes` : "PB changes unavailable"}</small>${r.end_ms != null && r.sort_ms !== r.end_ms ? `<small>Played ${esc(date(r.start_ms ?? r.end_ms,r.timezone))} · PBs at capture</small>` : ""}${versionChange ? '<span class="capture-note">Game version changed</span>' : ''}${missing.length ? '<span class="capture-note">Some original inputs unavailable</span>' : ''}</div><div class="rating"><small>Rating ${number(r.before_rating)} →</small><strong>${number(r.after_rating)}</strong><span class="gain${delta < 0 ? ' negative' : ''}">${signed(delta)}</span></div><div class="pools"><div><span class="old-label">Old 35</span><span class="pool-value">${number(r.after_old)}</span><small>${signed(change(r.before_old,r.after_old))} between snapshots</small></div><div><span class="new-label">New 15</span><span class="pool-value">${number(r.after_new)}</span><small>${signed(change(r.before_new,r.after_new))} between snapshots</small></div></div><span class="arrow" aria-hidden="true">›</span></a></li>`;
